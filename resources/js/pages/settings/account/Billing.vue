...
const workspacesLabel = computed(() =>
    transChoice('billing.plan.workspaces', props.workspaceCount, {
        count: String(props.workspaceCount),
    }),
);

const monthlyPrice = computed(() =>
    props.plan
        ? trans(`billing.subscribe.prices.${props.plan.slug}.monthly`)
        : '',
);
const yearlyPerMonthPrice = computed(() =>
    props.plan
        ? trans(`billing.subscribe.prices.${props.plan.slug}.yearly_per_month`)
        : '',
);

const showAnnualBanner = computed(
    () => props.hasSubscription && !isYearly.value,
);
                ...
                >
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-1 text-sm font-semibold text-foreground">
                            <span
                                class="text-foreground/50 tabular-nums line-through"
                                >{{ monthlyPrice }}</span
                            >
